...
class AppointmentServiceImplTest {
    ...
    // 12. Reschedule appointment success
    @Test
    void rescheduleAppointment_InternalServerError() {
        mockIsAdmin(false);
        ...
        existingAppointment.setStartTime(LocalTime.of(9, 0));
        existingAppointment.setEndTime(LocalTime.of(10, 0));
        existingAppointment.setStatus(StatusEnum.BOOKED);

        AppointmentDTO newAppointmentDTO = new AppointmentDTO();
        newAppointmentDTO.setDate(LocalDate.now().plusDays(1));
        newAppointmentDTO.setStartTime(LocalTime.of(11, 0));
        newAppointmentDTO.setEndTime(LocalTime.of(12, 0));
        ...
        when(availabilityRepository.findOverlappingAvailabilities(anyLong(), any(), any(), any()))
                .thenReturn(List.of(new Availability())); // Available slot
        when(appointmentRepository.existsBookedAppointment(anyLong(), any(), any(), any())).thenReturn(false);
        when(appointmentRepository.save(any())).thenAnswer(invocation -> invocation.getArgument(0));
        when(globalMapper.toAppointmentDTO(any())).thenReturn(newAppointmentDTO);

        ResponseEntity<?> response = service.rescheduleAppointment(newAppointmentDTO, 1L, patient.getUsername());

        assertEquals(500, response.getStatusCodeValue());
        verify(appointmentRepository).save(any());
    }

    // 14. Cancel appointment unauthorized
    @Test
    void cancelAppointment_Unauthorized() {
        ...
        ResponseEntity<?> response = service.cancelAppointment(1L, patient.getId(), "otherUser");

        assertEquals(401, response.getStatusCodeValue());
        assertEquals(UNAUTHORIZED, response.getBody());
    }

    // 10. View all appointments for patient unauthorized
    @Test
    void viewAllAppointmentsForPatient_Unauthorized() {
        mockIsAdmin(false);

        when(patientRepository.findById(anyLong())).thenReturn(Optional.of(patient));

        ResponseEntity<?> response = service.viewAllAppointmentsForPatient(patient.getId(), "otherUser", 0, 10);
